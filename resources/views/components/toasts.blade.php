...
<div {{ $attributes->class('contents') }} x-data="window.ToastyComponent(@js($componentConfig), @js($initialToasts))" x-init="boot()">
    <template x-teleport="body">
        <ul
            ...
            class="fixed block w-full group sm:max-w-xs"
            :style="{ zIndex: zIndex }"
        ...
        >
            <template x-for="toast in toasts" :key="toast.id">
                ...
                >
                    <span
                        class="relative flex flex-col items-start w-full transition-all duration-300 ease-out bg-white border border-gray-100 shadow-[0_5px_15px_-3px_rgb(0_0_0_/_0.08)] sm:rounded-md sm:max-w-xs group"
            ...
            window.ToastyComponent = function (config = {}, initialToasts = []) {
                return {
                    ...
                    showFromEvent(detail = {}) {
                        console.debug('[Toasty] event received', detail);
                    ...
                    enqueueToast(detail = {}) {
                        const toast = {
                            ...
                            type: ['default', 'success', 'info', 'warning', 'danger'].includes(detail.type) ? detail.type : 'default',
